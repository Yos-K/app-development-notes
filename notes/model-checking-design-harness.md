...
## 探索と形式モデルを突合して「守備範囲の穴」を見つける

形式モデルは強力だが、**何を形式化したかを忘れると過信を生む**。モデルが緑でも、それは
「モデルが対象とした範囲」の保証でしかない。探索テスト（実挙動を probe で観測する活動）の
観測結果とモデルの保証を**突き合わせる**と、両者の境界が見える。

```mermaid
flowchart LR
    Probe[探索: 実挙動の観測] --> Cross{突合}
    Model[形式モデル: 保証] --> Cross
    Cross -->|一致| Confidence[範囲内の確信]
    Cross -->|モデル外の観測| Gap[守備範囲の穴]
    Gap --> Extend[モデルを拡張]
```

実例: ある購入状態モデルは「永続化コード↔権限導出」を形式化し、全保証が成立していた。だが
探索で観測した**最も収益クリティカルな不変条件**（外部課金の「承認済みでなければ確定購入にしない」
境界）は、モデルの対象外だった。永続化の層だけがモデル化され、外部境界の安全側変換は単体テスト
**だけ**が守っていた。突合がこの穴を可視化し、その境界（`(状態, 承認フラグ) → 購入状態` の写像と
「Pro に到達できるのは承認済み購入だけ」という到達可能性の主張）をモデルに追加できた。

学び:

- **「モデルが緑」は「重要なことが全部守られている」ではない**。緑なのはモデル化した命題だけ。
- **探索（帰納: 実例から）と形式検査（演繹: 全数から）は補完的**。探索は穴の在り処を指し、モデルは
  指された穴を全数で塞ぐ。一方だけでは「重要なのに未検査」が残る。
- **安全クリティカルな不変条件は、観測・単体テスト・形式モデルの三層で守る**のが理想。三層のどれに
  穴があるかは、突合してはじめて分かる。
