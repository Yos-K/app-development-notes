...
## SURVIVED ランキングの2つの読み方

SURVIVED が多いクラスのランキングは、実は性質の異なる2種類の検出器の合成になっている。同じ対処をすると消耗する。

```mermaid
flowchart TD
    Surv[SURVIVED 上位クラス]
    Surv --> A[複雑度起因<br/>認知的複雑度も突出して高い]
    Surv --> B[観測可能性起因<br/>メソッド単体は単純なのに生き残る]
    A --> Fix1[対処: リファクタリング<br/>分解すればテストが追いつく]
    B --> Fix2[対処: 証明つきで台帳に記録<br/>テスト追加では殺せない]
```

- **複雑度起因**: 巨大メソッドは分岐の組合せが多すぎてテストが覆い切れない。SURVIVED 数と認知的複雑度が同時に突出する。これは複雑度メトリクスと相互検証でき、**リファクタリングで直すべきシグナル**。
- **観測可能性起因**: メソッド単体は単純（認知的複雑度 3〜5 程度）なのに SURVIVED が多い場合、原因は複雑さではなく「変異の効果が公開 API から観測できない」ことにある。実例として、幾何判定の2つの比率が相補（和=1.0）になっていて片方の違反が常にもう片方の違反を伴う（＝片方の判定は結果を変えられない）ケースや、固定データセット越しの順序比較しか公開されていない数値演算、float の厳密等値にしか反応しない境界変異がある。

後者をテストで殺そうとすると、実装専用の public API を生やすか、float の2進近似と格闘することになり、どちらも本末転倒になる。

## 等価変異は「証明つきで記録」する

「等価変異は記録して除外」の運用を一歩進めて、**なぜ殺せないかの証明を台帳に残し、除外はしない**形が良かった。

| 項目 | 内容 |
|------|------|
| 記録するもの | 対象クラス・変異の種類・**等価と判断した根拠（証明）** |
| 除外しない理由 | 公開 API が変われば観測可能になりうる。計測は続け、判断だけ記録する |
| 再評価のトリガー | 該当クラスの公開 API が変わったとき |

根拠は「観測できる入力が構成できないことの証明」まで書く。「たぶん等価」で台帳に入れると、台帳がスコア逃避の隠れ蓑になる。逆に証明を書こうとすると、上の相補比率のような**設計の冗長性（実は片方の条件が不要）**が見つかることがあり、台帳作成自体が設計レビューとして機能する。

関連: [ハーネスエンジニアリングで学んだこと](harness-engineering.md) / [性質ベーステスト (PBT) で学んだこと](property-based-testing.md) / [AI改修コストをコード品質で測る](code-quality-metrics-for-ai.md)
